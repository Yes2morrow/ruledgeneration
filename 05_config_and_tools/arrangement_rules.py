...
from module_e_organizer import organize_module_e
from module_f_organizer import organize_module_f
...
def generate_group_arrangement(module_type, quantity):
    ...
    # 对于模块E，优先使用双模块镜像组团
    if module_type == 'E':
        return organize_module_e(quantity)

    # 对于模块F，优先使用 4 模块 2x2 对称组团
    if module_type == 'F':
        return organize_module_f(quantity)

    # 模块G不做组团，每个模块都作为独立单元参与布局。
    if module_type == 'G':
        return [
            {
...
def get_module_spacing_config(module_type, space_type='均衡型'):
    ...
    if module_type == 'A':
        ...
        return {'vertical_gap': 0, 'horizontal_gap': 1.5}
    elif module_type == 'E':
        return {'vertical_gap': 1.5, 'horizontal_gap': 0}
    elif module_type == 'F':
        return {'vertical_gap': 0, 'horizontal_gap': 1.5}
    elif module_type == 'G':
        return {'vertical_gap': 1.5, 'horizontal_gap': 1.5}
    else:
        return {'vertical_gap': 0, 'horizontal_gap': 0}
