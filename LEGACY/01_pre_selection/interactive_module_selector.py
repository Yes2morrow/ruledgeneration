...
def check_layout_feasibility(module_selection, aisle_width, all_length, all_width):
    ...
    layout_result = calculate_layout(group_configs, aisle_width, all_width, all_length, modules_selection=module_selection)
    return layout_result is not False
...
def interactive_module_selection(space_type, N, all_length, all_width, auto_select=False,
    ...
    try:
        ...
        # 直接传递米单位的尺寸，让calculate_layout函数内部处理单位转换
        recommended_layout = select_modules(module_preferences, N, all_length, all_width)
