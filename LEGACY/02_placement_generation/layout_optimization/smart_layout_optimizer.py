...
from module_spacing_manager import build_axis_search_candidates, determine_inter_module_spacing, should_apply_inter_module_spacing
...
def find_best_position(group, current_positions, all_length, all_width, group_spacing=1.0):
    """
    ...
    for i, (x, y) in enumerate(normal_positions):
        normal_fits = can_place_group(x, y, normal_width, normal_height, current_positions, all_length, all_width, group_spacing)
        if normal_fits:
    ...
    for i, (x, y) in enumerate(rotated_positions):
        rotated_fits = can_place_group(x, y, rotated_width, rotated_height, current_positions, all_length, all_width, group_spacing)
        if rotated_fits:
    ...
    def calculate_compactness_score(candidate):
        ...
        if not current_positions:
            return (0, 0, 0)  # 如果没有已放置的群组，返回最优得分
        
        # 计算位置得分：优先选择左上角，但x坐标权重更高
        # 1. 位置得分：x坐标越小越好，然后y坐标越小越好
        position_score = (x, y)
        
        # 2. 空隙填补得分：检查是否能够填补现有布局中的空隙
        gap_fill_score = 0
        ...
        if left_adjacent and top_adjacent:
            gap_fill_score = 0  # 最优：左侧和上方都有群组
        elif left_adjacent or top_adjacent:
            gap_fill_score = 1  # 次优：一侧有群组
        else:
            gap_fill_score = 2  # 最差：独立位置
        
        return (gap_fill_score, position_score[0], position_score[1])
...
def can_place_group(x, y, group_width, group_height, current_positions, all_length, all_width, group_spacing):
    """
    ...
    for i, pos_info in enumerate(current_positions):
        ...
        # 检查矩形重叠（考虑群组间距）
        if group_spacing > 0:
            # 有间距要求：新群组需要与已放置群组保持group_spacing的距离
            ...
            # 无间距要求（group_spacing=0）：只检查是否真正重叠，允许紧贴
            overlap = not (x >= existing_x + existing_width or
                          x + group_width <= existing_x or
                          y >= existing_y + existing_height or
                          y + group_height <= existing_y)
        
        if overlap:
            if group_spacing > 0:
                print(f"    间距检查失败: 新群组({x:.2f}, {y:.2f}, {group_width:.2f}, {group_height:.2f}) 与已放置群组{i+1}({existing_x:.2f}, {existing_y:.2f}, {existing_width:.2f}, {existing_height:.2f}) 间距不足{group_spacing}m")
            else:
...
def calculate_smart_layout(groups_config, aisle, all_width, all_length, modules_selection=None):
    ...
    if modules_selection:
        ...
        # 如果有自适应间距设置，则覆盖默认的模块间间距
        if adaptive_spacing is not None and inter_module_spacing > 0 and adaptive_spacing < inter_module_spacing:
            inter_module_spacing = adaptive_spacing
            print(f"使用自适应间距覆盖默认模块间间距: {adaptive_spacing:.1f}m")
    else:
    ...
    while i < len(groups_config):
        ...
        if needs_inter_module_spacing:
            ...
            current_group_spacing = 0.0  # 模块A同类型群组间无间距
        elif mod['name'] == 'B':
