...
plt.rcParams['font.sans-serif'] = ['SimHei', 'WenQuanYi Zen Hei', 'Noto Sans CJK SC', 'Microsoft YaHei', 'DejaVu Sans']
    ...
    for group in groups_config:
        ...
        if group.get('subgroups'):
            ...
            if show_labels:
                ...
                label_text = f"{mod['name']}{group.get('module_count', 0)}"
                ax.text(
                    text_x,
                    text_y,
                    label_text,
        ...
        if group.get('unit_positions'):
            ...
            if show_labels:
                ...
                font_size = min(label_width, label_height) * 0.5
                font_size = max(6, min(font_size, 12))
                label_text = f"{mod['name']}{group.get('module_count', len(group['unit_positions']))}"
                ax.text(
                    text_x,
                    text_y,
                    label_text,
        ...
        if (
            mod['name'] == 'A' and group_type in ['A_group_one', 'A_group_two', 'A_group_three']
        ) or (
