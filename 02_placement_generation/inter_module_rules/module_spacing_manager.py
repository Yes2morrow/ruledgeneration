...
MANDATORY_INTER_MODULE_SPACING = 1.5
DEFAULT_INTER_MODULE_SPACING = MANDATORY_INTER_MODULE_SPACING
DEFAULT_GROUP_SPACING = 0.0
DEFAULT_SEARCH_GRID_STEP = 0.5
DEFAULT_AXIS_GRID_POINT_LIMIT = int(os.environ.get("LAYOUT_AXIS_GRID_POINT_LIMIT", "48"))
...
def determine_inter_module_spacing(modules_selection: Dict[str, int]) -> float:
    """
    当布局中同时存在不同类型模块时，统一要求至少保留 1.5m 间距。
    ...
    active_module_names = {
        module_name
        for module_name, quantity in (modules_selection or {}).items()
        if quantity
    }
    if len(active_module_names) >= 2:
...
def should_apply_inter_module_spacing(group1_module_name: str, group2_module_name: str) -> bool:
    ...
    :return: 是否需要间隔
    """
    return bool(group1_module_name) and bool(group2_module_name) and group1_module_name != group2_module_name


def get_pair_spacing_requirement(
    group1_module_name: str,
    group2_module_name: str,
    base_spacing: float = 0.0,
) -> float:
    """
    返回两个群组之间应满足的基础最小间距。

    规则：
    1. 不同类型模块之间，至少保留 1.5m。
    2. 同类型模块沿用调用方传入的原有间距。
    """
    required_spacing = max(0.0, base_spacing)
    if should_apply_inter_module_spacing(group1_module_name, group2_module_name):
        required_spacing = max(required_spacing, MANDATORY_INTER_MODULE_SPACING)
...
def build_axis_search_candidates(
    ...
    if include_grid and step_size > 0:
        ...
        if grid_count <= DEFAULT_AXIS_GRID_POINT_LIMIT:
            grid_indexes = range(grid_count + 1)
        else:
            sample_count = max(2, DEFAULT_AXIS_GRID_POINT_LIMIT)
            grid_indexes = {
                0,
                grid_count,
                *(round(i * grid_count / (sample_count - 1)) for i in range(sample_count))
            }
...
def get_spacing_info(modules_selection: Dict[str, int]) -> Dict:
    ...
    return {
        ...
        'spacing_rule': f"当前异类型模块最小间距为{inter_spacing}米",
...
def _get_spacing_description(type_ratios: Dict[str, float], spacing: float) -> str:
    ...
    """
    if spacing > 0:
        return f"当前不同模块类型之间统一至少保留{spacing}米间距"
    return "当前仅存在单一模块类型，不触发异类型模块间距规则"
